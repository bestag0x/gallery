...
function Frame({ url, text, desc, link, buyImage, c = new THREE.Color(), ...props }) {
  ...
  const buyImgUrl = '/15_normal.png' //selezione immagine hover copertina
  useCursor(hovered)
  useFrame((state, dt) => {
    //image22.current.material.zoom = 1 + Math.sin(rnd * 10000 + state.clock.elapsedTime / 3) / 2 //zoom in-out immagine copertina libro
    ...
    image.current.material.opacity = isActive && hovered ? 0.3 : 0 //immagine che appare quando con il mouse passi sopra una copertina
